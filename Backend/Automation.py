...
def CreateFolder(command_suffix: str):
    try:
        # Step 1: Find the true Windows user profile path
        user_profile = os.environ.get('USERPROFILE')
        
        # Step 2: Check standard Desktop vs OneDrive Desktop
        standard_desktop = os.path.join(user_profile, 'Desktop')
        onedrive_desktop = os.path.join(user_profile, 'OneDrive', 'Desktop')
        
        # Default to standard, but overwrite if OneDrive is active
        target_desktop = onedrive_desktop if os.path.exists(onedrive_desktop) else standard_desktop
        
        # Step 3: Construct the final folder path
        final_path = os.path.join(target_desktop, command_suffix)
        
        # Step 4: Execute the creation
        if not os.path.exists(final_path):
            os.makedirs(final_path)
            return f"Successfully created folder '{command_suffix}' on the Desktop."
        else:
            return f"Folder '{command_suffix}' already exists on the Desktop."

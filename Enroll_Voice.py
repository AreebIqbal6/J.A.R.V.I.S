...
# ===========================================
# --- WINERROR 1314 & 87 SYMLINK BYPASS ---
# ===========================================
...
def _safe_symlink(src, dst, target_is_directory=False, **kwargs):
    try:
        ...
        # Catch 1314 (Privilege), 87 (Invalid Parameter '.'), and 183 (Already Exists)
        if win_err in (1314, 87, 183):  
            src_str = str(src)
            dst_str = str(dst)
            
            # Infinite Loop Protection: DO NOT copy the current directory ('.') into itself
            if src_str in (".", "..") or os.path.abspath(src_str) == os.path.abspath(dst_str):
                return
                
            try:
                if os.path.isdir(src_str):
                    shutil.copytree(src_str, dst_str, dirs_exist_ok=True)
                else:
                    shutil.copy2(src_str, dst_str)
            except Exception:
                pass # If fallback copy fails, just proceed silently
        else:
            raise
...
def enroll_user():
    ...
    try:
        ...
        # Convert the raw microphone data directly into a PyTorch shape
        signal = torch.from_numpy(recording).squeeze().unsqueeze(0)
        ...
        traceback.print_exc() # Prints the exact line of failure

  ...
  function handleSocketMessage(type, d) {
    if (type === 'text_output') {
      ...
      if(chatLog){
        // d is a string: "User : Hello" or "JARVIS : Hi"
        const isUser = d.startsWith("User") || d.toLowerCase().includes(" : ") === false;
        const className = isUser ? 'user' : 'assistant';
        chatLog.innerHTML += `<div class="line ${className}">${d}</div>`;
        chatLog.scrollTop = chatLog.scrollHeight;
      }
    }
    else if (type === 'status_change') {
      const statusLbl = document.querySelector('.status-text') || document.querySelector('.central-status') || document.getElementById('jarvisStatus');
      if(statusLbl) statusLbl.textContent = d;
      // In the mockup, there might not be a dedicated status label with these exact IDs, 
      // but let's try to update any element that might represent the status.
      // E.g., JARVIS LIVE text
      const jLive = document.querySelector('.glitch');
      if(jLive && d) jLive.textContent = `JARVIS: ${d.toUpperCase()}`;
    }
    else if (type === 'swarm_message') {
      // d is {agent: "AGENT", message: "..."}
      const chatLog = document.querySelector('.chat-log');
      if(chatLog && d.message) {
        chatLog.innerHTML += `<div class="line" style="color: #888; font-size: 0.9em; font-style: italic;">[${d.agent}] ${d.message}</div>`;
        chatLog.scrollTop = chatLog.scrollHeight;
      }
    }
    else if (type === 'tts_waveform') {
      // Not fully implemented in 2D UI, but we can flash a border
      const p = document.getElementById('panelDynamic');
      if(p) {
        p.style.boxShadow = `0 0 ${d.intensity * 20}px rgba(0, 255, 255, 0.5)`;
        setTimeout(() => { p.style.boxShadow = ''; }, 200);
      }
    }
    else if (type === 'widget_command') {
      if(d.action === "set_core_color") {
        document.documentElement.style.setProperty('--color-primary', d.color);
      }
    }
